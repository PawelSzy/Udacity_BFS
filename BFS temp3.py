...
class Point:
    ...
    def new_direction(self, action):
        new_direction_index = (self.direction_index() + action) % len(forward_name)
        return forward[new_direction_index], forward_name[new_direction_index]
    
    def direction_to_point(self, next_point):
    ...
    def cost_to_goal(self, grid, goal, cost):
        len_x_grid = len(grid) 
        ...
        while len(frontier)!=0:
            point = frontier.pop()
            visited.append(point.return_X_Y())
            for kierunek in forward:
                ...
                if is_valid_point(next_point, len_x_grid, len_y_grid) and next_point not in visited :
                    ...
                    new_cost, action, new_direction, index = self.cost_between_points(next_point, cost)
                    new_cost = point.cost + new_cost
                    
                    new_direction, direction_name = point.new_direction(action)
                    new_point = Point( x,y,direction = direction_name, point_cost=new_cost, steps_from_goal = 0)
                    if next_point == goal:
                        return new_cost
    ...
    def cost_between_points(self, next_point, cost):
        ...
        i = self.direction_to_point(next_point)
    ...
    def min_value_of_neighbors(self, grid, value):
        ...
        for kierunek in forward:
            ...
            if is_valid_point(next_point, len_x_grid, len_y_grid) and (value[next_point[0]][next_point[1]] != '|') and (value[next_point[0]][next_point[1]] != '*') :
                next_x, next_y = next_point[0], next_point[1]
                #print value[next_x][next_y]
                if min_value > float(value[next_x][next_y]):
                    min_value, min_point = value[next_x][next_y], [next_x, next_y]
...
def optimum_policy2D(grid,init,goal,cost):
    ...
    #print initial_point
    cost_matrix[init[0]][init[1]] = initial_point.cost_to_goal(grid, goal, cost)
    visited = []
    #cost.reverse()
    while len(frontier)!=0:
        point = frontier.pop()
        ...
        for kierunek in forward:
            ...
            if is_valid_point(next_point, len_x_grid, len_y_grid) and next_point not in visited :
                ...
                x,y = next_point
                new_cost, new_action, new_action_name, move_index = point.cost_between_points(next_point, cost)
                ...
                new_direction, direction_name = point.new_direction(new_action)
                new_cost += point.cost
                new_point = Point( x,y,direction = direction_name, point_cost=new_cost, steps_from_goal = new_steps)
                cost_matrix[x][y] = new_point.cost_to_goal(grid, goal, cost)
                frontier.append(new_point)
    ...
    while len(frontier)!=0:
        point = frontier.pop()
        visited.append([point.x, point.y])
        print point.min_value_of_neighbors(grid, cost_matrix)
        if point.return_X_Y() == goal:
            value[point.x][point.y] =0 
            policy2D[point.x][point.y] = '*'
            break
        min_value, next_point = point.min_value_of_neighbors(grid, cost_matrix) 
        #print point.direction_to_point(next_point)
        #print action_name[point.direction_to_point(next_point)]
        policy2D[point.x][point.y] = action_name[point.direction_to_point(next_point)]
        
        
        #new_point = Point( x,y,direction = 'up')
        #if is_valid_point(next_point, len_x_grid, len_y_grid) and next_point not in visited:
            #frontier.append(new_point)
        for kierunek in forward:
            next_point = [x_or_y + strona_swiata for x_or_y, strona_swiata in zip(point.return_X_Y() ,kierunek) ] 
            ...
            if is_valid_point(next_point, len_x_grid, len_y_grid) and next_point not in visited :
                ...
                new_cost, new_action, new_action_name, move_index = point.cost_between_points(next_point, cost)
                new_direction, direction_name = point.new_direction(new_action)
                new_point = Point( x,y,direction = direction_name)
                #cost_matrix[x][y] = new_point.cost_to_goal(grid, goal, cost)
                frontier.append(new_point)
                
    return value, cost_matrix, policy2D
    return policy2D



policy, cost_matrix, policy2D = optimum_policy2D(grid,init,goal,cost)
for row in policy:
    print row
